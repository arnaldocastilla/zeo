...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 25-05-2018 a las 07:09:00
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_Consultorios` ()  BEGIN
    ...
    FROM consultorio c
    INNER JOIN departamentos d ON c.departamento = d.idDepartamento
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listarEtapaTumor` ()  BEGIN
	SELECT et.idEtapatumor, et.nombreetapa, CONCAT(p.identificacion, ' ', p.nombre, ' ', p.apellido) AS 'paciente', 
           ...
           CONCAT(m.codigom, ' ', m.nombrem) AS 'metastasis', ct.nombreclasificacion, et.diagnostico
    FROM etapatumor et
    INNER JOIN pacientes p ON et.Paciente = p.idPaciente
    INNER JOIN tumorprimario tp ON tp.idTumorprimario = et.Tumorprimario
    INNER JOIN ganglioslinfaticos gl ON et.Ganglioslinfaticos = gl.idGanglioslinfaticos
    INNER JOIN metastasis m ON et.Metastasis = m.idMetastasis
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_RegisterMedico` (IN `sp_tipoidentificacion` CHAR(2), IN `sp_identificacion` BIGINT, IN `sp_departamentoidentificacion` VARCHAR(35), IN `sp_nombre` VARCHAR(25), IN `sp_apellido` VARCHAR(35), IN `sp_apellidocasada` VARCHAR(35), IN `sp_genero` CHAR(1), IN `sp_fechanacimiento` DATE, IN `sp_tiposangre` CHAR(4), IN `sp_telefono` BIGINT(7), IN `sp_celular` BIGINT(10), IN `sp_estadocivil` VARCHAR(35), IN `sp_religion` VARCHAR(50), IN `sp_Departamento` INT(2), IN `sp_Municipio` INT(6), IN `sp_domicilio` VARCHAR(120), IN `sp_email` VARCHAR(200), IN `sp_clave` VARCHAR(60), IN `sp_estado` INT(1))  BEGIN
	INSERT INTO medicos (tipoidentificacion, identificacion, departamentoidentificacion, nombre, apellido, apellidocasada,
						 ...
						 Departamento, Municipio, domicilio, email, clave, estado) 
	VALUES (sp_tipoidentificacion, sp_identificacion, sp_departamentoidentificacion, sp_nombre, sp_apellido, sp_apellidocasada,
			sp_genero, sp_fechanacimiento, sp_tiposangre, sp_telefono, sp_celular, sp_estadocivil, sp_religion,
			sp_Departamento, sp_Municipio, sp_domicilio, sp_email, sp_clave, 1); 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_RegisterPaciente` (IN `sp_tipoidentificacion` CHAR(2), IN `sp_identificacion` BIGINT, IN `sp_departamentoidentificacion` VARCHAR(35), IN `sp_nombre` VARCHAR(25), IN `sp_apellido` VARCHAR(35), IN `sp_apellidocasada` VARCHAR(35), IN `sp_genero` CHAR(1), IN `sp_fechanacimiento` DATE, IN `sp_tiposangre` CHAR(4), IN `sp_telefono` BIGINT(7), IN `sp_celular` BIGINT(10), IN `sp_estadocivil` VARCHAR(35), IN `sp_ocupacion` VARCHAR(80), IN `sp_religion` VARCHAR(50), IN `sp_Departamento` INT(2), IN `sp_Municipio` INT(6), IN `sp_domicilio` VARCHAR(120), IN `sp_email` VARCHAR(200), IN `sp_clave` VARCHAR(60))  BEGIN
	INSERT INTO Pacientes (tipoidentificacion, identificacion, departamentoidentificacion, nombre, apellido, apellidocasada,
						   ...
						   Departamento, Municipio, domicilio, email, clave) 
	VALUES (sp_tipoidentificacion, sp_identificacion, sp_departamentoidentificacion, sp_nombre, sp_apellido, sp_apellidocasada,
			sp_genero, sp_fechanacimiento, sp_tiposangre, sp_telefono, sp_celular, sp_estadocivil, sp_ocupacion, sp_religion,
			sp_Departamento, sp_Municipio, sp_domicilio, sp_email, sp_clave); 
	SELECT 'registro ingresado con exito' as response;
 END$$
...
ALTER TABLE `actividades`
  ...
  ADD KEY `FK_Actividades_Pacientes_INDEX` (`Cita`),
...
ALTER TABLE `etapatumor`
  ...
  ADD KEY `FK_Etapatumor_Paciente` (`Paciente`);
...
ALTER TABLE `recetasmedicas`
  ...
  ADD KEY `FK_Recetasmedicas_Pacientes_INDEX` (`Cita`),
...
--
ALTER TABLE `auxiliares`
  MODIFY `idAuxiliar` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
--
ALTER TABLE `actividades`
  ADD CONSTRAINT `FK_Actividades_Citas` FOREIGN KEY (`Cita`) REFERENCES `cita` (`idCita`) ON DELETE CASCADE ON UPDATE CASCADE,
...
--
ALTER TABLE `consultorio`
  ADD CONSTRAINT `consultorio_ibfk_1` FOREIGN KEY (`departamento`) REFERENCES `departamentos` (`idDepartamento`),
  ADD CONSTRAINT `consultorio_ibfk_2` FOREIGN KEY (`municipio`) REFERENCES `municipios` (`idMunicipio`);
...
ALTER TABLE `etapatumor`
  ...
  ADD CONSTRAINT `FK_Etapatumor_Paciente` FOREIGN KEY (`Paciente`) REFERENCES `pacientes` (`idPaciente`) ON DELETE CASCADE ON UPDATE CASCADE,
...
--
ALTER TABLE `recetasmedicas`
  ADD CONSTRAINT `FK_Recetasmedicas_Cita` FOREIGN KEY (`Cita`) REFERENCES `cita` (`idCita`),
